...
import com.example.placemark.PlacemarkerMemStore
import com.example.placemark.PlacedMarkData

val store = PlacemarkerMemStore()
...
fun addPlacemark() {
    ...
    if (title.isNotEmpty()) {
        val placemark = PlacedMarkData(title = title, description = description)
...
fun updatePlacemark() {
    ...
    if (id != null && store.findOne(id) != null) {
        ...
        if (title.isNotEmpty()) {
            val updated = store.update(PlacedMarkData(id = id, title = title, description = description))
